      ...
      if(resItem.objResponse.length>0){
        ...
        resItem.objResponse.map(val=>{
          arrItem.push({
            ...
            strName : val.strName,
            strType: val.strType,
            strFile: val.strFile ==="" ? NoImage : "data:image/jpeg;base64," + val.strFile,
            dblQty:val.dblQty
         })
        })
        setArrItem(arrItem);
      }
      //setImgSrc("data:image/jpeg;base64," + resItem.objResponse);
    } else {
      ...
      <div className="container3-fluid">
        <div className="row mt-2">
          ...
          <div className="col-1 mt-1 pr-1">
           <button type="button" class="btn btn-success" onClick={fnCart} style={{width:"97%",border:"none", backgroundColor:"#970647"}}><i class="fa fa-cart-arrow-down">Cart</i></button>
          </div>
        </div>
              ...
              <div style={{width:"100%", height:"fit-content", border:"solid 2px", borderRadius:"10px"}}>
                <div className="row mt-2">
                  <div className="col-12">
                    <div style={{width:"100%"}}>
                      <img className="shop-item-image" src={val.strFile} />
                    </div>
                  </div>
                </div>
                <div className="row">
                ...
                <div className="row">
                  <div className="col-12 mb">
                    <button type="button" onClick={()=>FnAddToCart(val.strItemCode, val.dblPrice,val.strName)} className="btn btn-success" style={{width:"100%",border:"none"}}>Add to cart <i class="fa fa-cart-arrow-down"></i></button>
                  </div>
                </div>
              </div>
            </div>
